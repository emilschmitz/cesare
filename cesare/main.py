...
class CESARE:
    def __init__(
        ...
        if config.get("evaluation", {}).get("enabled", False):
            eval_model = models.get("evaluator", models["agent"])  # Default to agent model if not specified
    ...
    def run_simulation(self, start_prompt: str | None = None):
        ...
        if self.evaluator:
            evaluation = self.evaluator.evaluate_instruction(instruction, self.history, run_id=parent_run_id)
    ...
    def _update_parent_run_metrics(self, parent_run_id: str, is_final: bool = False):
        ...
        metadata = {
            ...
            "total_history_length": len(self.history)
        }
