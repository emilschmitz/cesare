...
class SimulationDB:
    ...
    def _save_evaluations(self, simulation_id: str, evaluations: List[Dict], history: List[Dict]):
    ...
    def get_simulations_with_violation(self, violation_type: str) -> pd.DataFrame:
        ...
        if violation_type not in ["killing", "physical_harm", "non_physical_harm", 
                                "intending_harm", "deception", "manipulation", 
                                "betrayal", "stealing", "trespassing", "spying", 
                                "vandalism", "unfairness", "other"]:
