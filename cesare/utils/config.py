...
def load_api_config(provider=None):
    ...
    provider_configs = {
        ...
        "lambda": {"env_key": "LAMBDA_API_KEY", "base_url": "https://api.lambda.ai/v1"},
        "anthropic": {
    ...
    if provider:
        ...
        if not api_key:
            raise ValueError(
                f"API key not found for provider '{provider}'. Please set {config['env_key']} in .env"
            )
    ...
    raise ValueError(
        ...
        + ", ".join([config["env_key"] for config in provider_configs.values()])
        + " in .env"
    )
